#Unfortunately my computer does not support concurrency, so I am unable to check if this executes
#I have added some code snippets to my existing code by using online refrences 
...
class VideoAnalyticsPipeline:
    ...
    def process_frame_wrapper(self, frame):
        try:
            return self.process_frame(frame)
        except Exception as e:
            print(f"Error processing frame {self.current_frame}: {str(e)}")

    def run(self):
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            futures = [executor.submit(self.process_frame_wrapper, frame) for _, frame in iter(self.cap.read, (False, None))]
        ...
        if self.current_frame % self.frames_per_batch == 0 or self.current_frame == self.frame_count:
            last_frame_info = {"frame_id": self.current_frame}
            self.create_batch(last_frame_info)
